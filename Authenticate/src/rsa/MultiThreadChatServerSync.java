...
public class MultiThreadChatServerSync{
 ...
 //generate public and private keys for server
   ...
   private static final KeyPair keyPairs = rsa.rsaKeyGen();
   public static PublicKey publicKey = keyPairs.getPublic();
   static PrivateKey privateKey = keyPairs.getPrivate();
   //generate master key
...
class clientThread extends Thread{
   ...
   private ArrayList<byte[]> sendEncrypted(byte[] nonce, byte[] iv){
	   ...
	   try{
		   ...
		   byte[] sign = rsa.sign(encrypted, this.prvKey);
            ...
            for(int i=0;i<maxClientsCount;i++){
               if(threads[i] !=null && threads [i] ==this){
                  ...
                  try {
                	  ...
                	  boolean verified = rsa.verifySign(encrypted, signed.remove(0), this.pubKey); //verify encrypted message came from client
                  ...
                  this.os.println("***********Seesion Key Info********");
                  this.session = server.getSession();
                  this.os.println("Session key: "+ Arrays.toString(this.session.getEncoded()) + " has been generated for the session" ); 
...
try {
	    ...
	    initVector = encryptor.initialisationVector();
	    	...
	    	byte[] signed = rsa.encrypt(encrypted, this.prvKey);
              ...
              synchronized(this){
                 for(int i=0; i<maxClientsCount; i++){
                    if(threads[i] != null && threads[i].clientName != null){
						if (threads[i] != this) {
							byte[] verified = rsa.decrypt(signed, this.pubKey);
							threads[i].os.println("<"+name + "> "+ "Decrypted using sending client's public key:");
							threads[i].os.println("<"+name + "> "+ Arrays.toString(verified));
							
							byte[] decrypted = encryptor.decrypt(encrypted, initVector, threads[i].session.getEncoded());
							threads[i].os.println("<"+name + "> "+ "Decrypted using shared session key:");
							threads[i].os.println("<"+name + "> "+ Arrays.toString(decrypted));
